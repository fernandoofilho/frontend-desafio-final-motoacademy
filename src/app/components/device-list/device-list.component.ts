...
export class DeviceListComponent implements OnDestroy, OnInit {
  ...
  constructor(
    ...
    private storageFilterService: MemoryFilterService,
    ...
    combineLatest([
      ...
      .pipe(
        ...
        switchMap(([isAiSearch, isNormalSearch]) => {
          ...
          const storage = this.storageFilterService.getMemoryQuery();
          const manufacturer = this.manufacturerFilterService.getManufacturerQuery();
          ...
          if (isAiSearch) {
            ...
            this.filterManagerService.setExecuteAISearch(false);
            this.filterManagerService.setExecuteSearch(false);
            const query: searchQuery = { search, group, year, storage, manufacturer };
